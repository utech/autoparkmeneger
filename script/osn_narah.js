...
//АТП14608
if (wInfo.Npr_ID == 4762){ //Для Сороки О.А. нарахування за другою ставкою
	kilkistRob = sumTabel.Work_days_sum;
	wOperacii[0].N_Osn_narah += (kilkistRob / mbn[0].WorkDaysInMonth) * 950 * 0.5;
}
...
//----------------Премія
//АТП 14608
var query = new USqlQuery;
query.exec("SELECT Suma FROM agentski WHERE TabelNum_id=320");
if (query.next())
	wOperacii[0].N_Premia = query.value(0);
else
	wOperacii[0].N_Premia = 0;


//----------------Матеріальна допомога

//----------------Доплата
wOperacii[0].N_Doplata = 0;

	//Для водіїв
if (wInfo.Posada_id == 10) {
	//Три відсотки від виручки
	query.exec("SELECT sum(VartProdKvytkiv) FROM shlyahovyiLyst \
				WHERE VodiyTabelNum_id="+wInfo.Npr_ID+" \
					and year(ToZvitDate)="+wOperacii[0].CurDate_year+" \
					and month(ToZvitDate)="+wOperacii[0].CurDate_month);
	if ( query.next() )
		wOperacii[0].N_Doplata += query.value(0) * 0.03;
	
	//Врахування часу з розривом
	query.exec("SELECT sum(time_to_sec(s.ChasVNaryadi)) FROM marshruty \
				LEFT JOIN (SELECT * FROM shlyahovyiLyst \
							WHERE VodiyTabelNum_id="+wInfo.Npr_ID+" \
								and year(ToZvitDate)="+wOperacii[0].CurDate_year+" \
								and month(ToZvitDate)="+wOperacii[0].CurDate_month+" \
							) AS s ON s.KodMarshrutu_id=marshruty.KodMarshrutu \
									and s.GrafikNum=marshruty.GrafikNum \
									and s.ZminaNum=marshruty.ZminaNum \
				WHERE marshruty.ChasZRozryvom=true;");
	if ( query.next() )
		wOperacii[0].N_Doplata += query.value(0) * 0.6 * wInfo.Stavka / 3600;
}
...
wOperacii[0].Vh_saldo = wOperacii[1].Vh_saldo + wOperacii[1].N_Osn_narah + wOperacii[1].N_VuslRokiv +
						...
						wOperacii[1].U_PensFond - wOperacii[1].U_Fond_zainat - wOperacii[1].U_SocStrah - 
